...
子 agent 会获得父 agent 的工作目录／会话谱系和输出 token 上限。其 provider、model 与显式 reasoning effort 来自父 agent 最新的 `request/header`；父 agent 尚无请求时回退到创建选项。`request.agentOptions` 可以覆盖 provider 与 model，路由发生变化时绝不继承父 agent 的 reasoning id。由 adapter 填充的 reasoning 默认值仍保持为默认值，不会固化为子 agent 选择。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
