...
export interface ContinuableSubagentDescriptorData extends SubagentDescriptorBase {
  ...
  /** Explicit reasoning effort captured for the resolved child route. */
...
function parseSubagentDescriptor(value: unknown): SubagentDescriptorData | undefined {
  ...
  const rawAgentReasoningEffort = optionalString(value, 'agentReasoningEffort')
  if (rawAgentReasoningEffort !== undefined && (agentProvider === undefined || agentModel === undefined)) {
    throw new Error('persisted subagent descriptor agentReasoningEffort requires agentProvider and agentModel')
  }
  const agentReasoningEffort = rawAgentReasoningEffort === undefined
    ? undefined
    : ReasoningEffortId(rawAgentReasoningEffort)
...
export function snapshotSubagentDescriptor(input: SubagentDescriptorInput): SubagentDescriptorData {
  const candidate: SubagentDescriptorData = input.mode === 'one-shot'
    ...
    : {
      ...
      ...input.agentReasoningEffort !== undefined
