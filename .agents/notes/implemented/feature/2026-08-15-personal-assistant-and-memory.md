...
**The assistant can coordinate the live registry.** Its scoped setup registers `list_sessions`, which returns live status and activity metadata, and `send_message_to_session`, which appends a sourced relay user message through the target agent's ordinary follow-up inbox. Ordinary coding sessions never see these tools or their prompt guidance. Persisted but non-live sessions remain the responsibility of `/resume` and are not implied by `list_sessions`.
...
Registering session-control tools for every coding session would give ordinary project agents coordination authority they do not need. Addressing persisted but inactive sessions through the assistant would duplicate `/resume` and bypass the live agent inbox, so coordination remains limited to the in-process registry. A background assistant daemon would require a separate lifecycle and is outside the terminal process model.
