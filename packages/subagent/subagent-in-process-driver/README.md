...
The child gets the parent's working-directory/session lineage and output-token cap. Its provider, model, and explicit reasoning effort come from the latest parent `request/header`, falling back to creation options before the parent's first request; `request.agentOptions` overrides provider and model, and a changed route never receives the parent's reasoning id. Adapter-materialized reasoning defaults remain defaults rather than fixed child selections. It gets a fresh flat registration scope: parent ownership does not import parent tool restrictions or establish an authority subset.
