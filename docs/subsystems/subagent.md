...
- **Model selection** is captured synchronously from the delegating parent's latest `request/header`, with parent creation options used only before any request exists. Explicit child provider/model fields win; reasoning is inherited only for the identical route and only when it was not an adapter-materialized default. A fork seed may contain an older request header, but the child-scoped capture determines the new child request. Continuable descriptors preserve that selection for cold resume instead of reading the parent's later state.
